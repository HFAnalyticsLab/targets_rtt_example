...
dashboard_stats_ccg <- function(combined_dataset,
                                monthyear,
                                ccg_code = 'ENGLAND',
                                specialty,
                                type,
  ...
  x <- open_dataset(combined_dataset)
    ...
    if (ccg_code=='ENGLAND'){
      ...
      ccg_name <- filter(x, monthyr==monthyear&Commissioner.Org.Code==ccg_code) %>%
        select(.,Commissioner.Org.Name) %>% unique(.) %>% unlist(.) %>% first(.) |> collect()
    }
